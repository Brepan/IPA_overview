...
\thispagestyle{empty}
\section*{Úvod}
Inspirován bindiff tahákem, jal jsem se v rámci učení si taky vytvořit
vlastní tahák/overview. Není to dokončeno, ale je tu snad vše z
MMX, SSE a AVX. ARM je jen popsaný a s pár funky instrukcemi
a Neon není vůbec. Tak zveřejňuji, třeba to někomu pomůže.
\\[0.5em]
Chyby? Možná jsou, nezaručuju, většina věcí je převzatá z prezentací,
jenom napsána jinak. Pokud něco najdete a budete mít zájem třeba expandovat
tuto srandu, zde odkaz na repo, dělejte si s ním co chcete:
% Jediné co zakazuji je smání se mému kódu,
% bludný cestovateli ....
\url{https://github.com/Brepan/IPA_overview}
...
Přesune dolních 64 bitů, horní bity nevynuluje.
\\[0.5em]
\instr{VMOVLP(S|D)} \reg{xmm}\textbf{,} \reg{xmm}\textbf{,} \mem{m64}\\
Funguje jako VMOVS(S|D), ale s pamětí a bez vynulování horních bitů.\\
Při zápisu do paměti má pouze dva operandy a funguje očekávaným způsobem.
\\[0.5em]
...
Podobné přesunům dolní poloviny, ale operuje na horních 64 bitech registru XMM.
\\[0.5em]
...
LH $\rightarrow$ dH = s2L, dL = s1L\\
HL $\rightarrow$ dH = s1H, dL = s2H
\\[0.5em]
...
\includegraphics[width=0.4\textwidth]{pics/EXTR.png}
\\[0.5em]
...
\includegraphics[width=0.4\textwidth]{pics/INSER.png}
\\[0.5em]
...
\includegraphics[width=0.8\textwidth]{pics/ALIGN.png}\\
...
Zpackuje(zmenší) dvojslova z dst i src do unsigned wordů v dst.
\\[0.5em]
...
nastavený bit.\\
\includegraphics[width=0.8\textwidth]{pics/DP.png}
\\[0.5em]
...
\includegraphics[width=1\textwidth]{pics/MPSA.png}
\\[0.5em]
...
Jako SHUF prostě, ale dynamicky lze.\\
...
V prezentaci jsou na to tabulky, sem jsem to kopírovat už nechtěl.
\\[0.5em]
...
32. registr je xzr, tedy nulový registr.
...
U 32bit je pak návrat MOV pc,lr (64bit má normálně RET)\\
...
nic moc extra šílenýho jako v SSE a AVX tam není, i když určitě doporučuju si projít, jelikož
...
\section{ARM Neon}
\dots nedoděláno, opět v prezentaci docela fajn, hlavně bacha
na ty load a store funkce jsou docela komplexní, zbytek bych řekl, že
je intuitivní.

\newpage
\section{Jaké bylo ústní zkoušení}
Abych pokračoval v tradici, hodím sem jak vypadal zkouška v 2026.\\
Můžu mluvit asi jenom za ústní, jelikož to jsem měl a s nikým kdo psal
zkoušku jsem se moc nepotkal.

Měl jsem FoGa, povídali jsme 10-20 minut asi.\\
Dal mi na výběr kde chci začít, vybral jsem si ARM, mega jsem to pokazil,
pak jsme šli na MMX a SSE, to už bylo lepší.\\

Hodně se ptal na obecné věci, jako kolik to má registrů, čím se to vyznačuje,
atd.,takže to se určitě hodí vědět.
Instrukce jsem měl LDR a STR u ARM, a chtěl to se vším všudy, včetně
indexování atd. Potom SHUF a DPP (dot product). Takže řekl bych
větší část byla teorie.

Ostatní co jsem se ptal, tak měli taky hodně obecných otázky spíše.
SHUF prej padal hodně, LDR, STR taky. Párkrát se zeptali i na WebAssembly,
nebo na ARM 64. Prý padlo i GATHER u někoho.

Obecně to ale bylo chill, já jsem v ARMu hrozně zmatkoval a jako body
sebral na nějakých 55\% za tu ARM část (části se hodnotí zvlášť),
ale mohlo být mnohem hůř podle mě.
Všichni co jsme tam tehdy byly(tak 10 lidí), tak jsme asi prolezli, co jsem tak odposlouchal.
\end{document}